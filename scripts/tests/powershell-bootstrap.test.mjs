...
assert.match(bootstrap, /'pnpm install --frozen-lockfile'\.Split\(' '\)/);
assert.match(bootstrap, /\('exec', '--'\) \+ \$pnpmInstallCommand/);
assert.match(bootstrap, /'uv sync --project crewai --frozen'\.Split\(' '\)/);
assert.match(bootstrap, /\('exec', '--'\) \+ \$uvSyncCommand/);
