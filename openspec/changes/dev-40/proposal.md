...
a measured baseline for token-limit calibration.
...
| --- | --- |
| AC-001 Persist phase contracts and reuse valid planning on repair | Persisted phase contracts |
...
| AC-004 Reject stale inputs and write state atomically | Hash validation and atomic execution state |
| AC-005 Give Reviewer complete, compact, hash-backed evidence | ReviewPack evidence |
| AC-006 Persist bounded role usage for token calibration | Token usage observability |
