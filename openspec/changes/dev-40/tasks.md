...
| AC-006 Invoke isolated one-task roles without delegation, bound planning context, file, and Linear output, and persist configured token usage | 2. Model and persist workflow contracts; 3. Isolate CrewAI role invocations; 4. Implement the persistent phase machine | `crewai/tests/test_planning.py`; `crewai/tests/test_crew.py`; `crewai/tests/test_main.py`; `crewai/tests/test_custom_tool.py` |
...
- [ ] **3. Isolate role invocations and bound outputs**
  ...
  - Add coverage in `crewai/tests/test_crew.py` for role isolation, bounded
    configuration, staged Architect inputs, exact structured outputs, and
    disabled delegation.
