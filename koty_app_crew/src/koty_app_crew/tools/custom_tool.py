...
def ejecutar_openspec(comando_openspec: str) -> str:
    ...
    try:
        ...
        return "Error: El comando tardó más de 30 segundos y se congeló. Usa otro comando."
    except Exception as e:
...
def leer_archivo_raiz(ruta_relativa: str) -> str:
    """
    Lee un archivo garantizando que se busque en la RAÍZ del proyecto.
    Solo debes pasar el nombre (ej: 'CONTEXT.md' o 'apps/web/package.json').
    """
    try:
        ruta_completa = os.path.abspath(os.path.join("..", ruta_relativa))
